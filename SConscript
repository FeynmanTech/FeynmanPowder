...
AddSconsOption('release', True, True, "Enable loop / compiling optimizations (default).")
...
env.Append(CPPPATH=['src/', 'data/', 'generated/', '/usr/include/luajit'])
if GetOption("msvc"):
	if GetOption("static"):
...
def findLibs(env, conf):
	...
	if platform == "Windows":
		if msvc:
			...
			if False: #not conf.CheckLib('mingw32') or not conf.CheckLib('ws2_32'):
				FatalError("Error: some windows libraries not found or not installed, make sure your compiler is set up correctly")
	...
	# 	#Look for LuaJit
	# 	if not conf.CheckLib(['libluajit-5.1','libluajit','luajit', 'luajit-2.0']):
	# 		FatalError("luajit development library not found or not installed")
	# 	if platform == "Linux":
	# 		try:
	# 			env.ParseConfig("pkg-config --cflags luajit")
	# 			env.ParseConfig("pkg-config --libs luajit")
	# 		except:
	# 			pass
	# 		#Look for luajit.h
	# 	if not conf.CheckCHeader('luajit.h'):
	...
	if not GetOption('nolua') and not GetOption('renderer') and GetOption('nojit'):
		#Look for Lua
		luaver = "lua5.1"
	...
	if not GetOption('nolua') and not GetOption('nojit'):
		#Look for LuaJit
		if not conf.CheckLib(['libluajit-5.1','libluajit','luajit', 'luajit-2.0']):
			FatalError("luajit development library not found or not installed")
		if platform == "Linux":
	...
	if not GetOption('nofft') and not conf.CheckLib(['fftw3f', 'fftw3f-3', 'libfftw3f-3']):
			print("fftw3f development library not found or not installed")

	#Look for bz2
	if not conf.CheckLib(['bz2', 'libbz2']):
		print("bz2 development library not found or not installed")
	...
	if not conf.CheckLib(['pthread', 'pthreadVC2']):
		print("pthreads development library not found or not installed")
	...
	if msvc:
		...
		if not conf.CheckLib('m'):
			print("libm not found or not installed")
	...
	if platform == "Linux":
		if not conf.CheckLib('X11'):
			print("X11 development library not found or not installed")

		if not conf.CheckLib('rt'):
			print("librt not found or not installed")
	elif platform == "Windows":
...
if GetOption('clean'):
	...
	if False: #not conf.CheckCC() or not conf.CheckCXX():
...
if not GetOption('nolua') or not GetOption('renderer'): #as if I would ever want to compile without lua
	env.Append(CPPDEFINES=['LUACONSOLE'])

if GetOption('tptnet'):
	env.Append(CPPDEFINES=['TPTNET'])
...
if not GetOption('nolua') or not GetOption('renderer'):
	sources += Glob("src/lua/socket/*.c") + Glob("src/lua/LuaCompat.c")

#sources += Glob("-L/usr/include/luajit")
