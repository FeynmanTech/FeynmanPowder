...
int SDLOpen()
{
	...
	SDL_WM_SetCaption("Feynman's Mod", "FeynmanPowder");
...
bool SaveWindowPosition()
{
	...
	if (SDL_GetWMInfo(&sysInfo) > 0)
	{
		...
		if (y < 100) // Sometimes is created behind taskbar (if taskbar is on the top of the screen), rendering it immovable
			y = 100;

		Client::Ref().SetPref("WindowX", (int)placement.rcNormalPosition.left);
		Client::Ref().SetPref("WindowY", y);
