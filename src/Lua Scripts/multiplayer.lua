function tpt.notify(title, message, time)
    if jit.os == 'Linux' then
        os.execute("notify-send '" .. title .. "' '" .. message .. "' -t 1250")
    end
end
...
mpapi.chatbuffer = ""
mpapi.print = function(...) chatwindow:addline(...) end
...
local function disconnected(reason, isQuit)
    if con.socket then
        con.socket:close()
    end
    con.connected = false
    ...
    if reason then
        ...
        if mpList['autoconnect'] ~= nil and mpList['autoconnect'] ~= 'false' and not isQuit then
...
local function conGetLen()
    con.socket:settimeout(nil)
    local c,r = con.socket:receive(1)
    if not c and r ~= "timeout" then disconnected() return nil end
    local rstring=""
    local len = c:byte()
    rstring,r = con.socket:receive(len)
    if not c and r ~= "timeout" then disconnected() return nil end
    con.socket:settimeout(0)
    return rstring
end
local function conGetLine()
    con.socket:settimeout(nil)
    local rstring=""
    rstring,r = con.socket:receive("*l")
    if not c and r ~= "timeout" then disconnected() return nil end
    con.socket:settimeout(0)
    return rstring
end
...
new=function(x,y,w,h)
    ...
    chatcommands = {
    notify = function(self, msg, args)
        if args[1] then
            mpAdd("notify", "all")
        elseif mpList.notify == "true" then
            mpAdd("notify", "false")
            self:addline("Turned notifications off", 255,50,50)
        else
            mpAdd("notify", "true")
            self:addline("Turned notifications on", 50,255,50)
        end
    end,
	...
	quit = function(self,msg,args)
        if #args > 0 then
            ...
            chatwindow:addline("* " .. CurrentNick .. " is leaving (" .. table.concat(args, ' ') .. ")", 255, 125, 50)
        end
	...
	afk = function(self, msg, args)
        if isAFK then
            ...
            if type(args[1]) == "number" then
                ext = ", and should be back in about " .. args[1] .. " minutes."
            elseif args[1] then
                ext = " (" .. table.concat(args, " ") .. ")"
            end
            parseAndSend('/me is now AFK' .. ext, self)
    ...
    hidden = function(self, msg, args)
        --table.insert(args, 1, string.char(#(table.concat(args, ' '))))
        local s = table.concat(args, ' ') .. string.char(10)
        --[[
        conSend(nil, string.char(55))
        for n = 1, s:len() do
            conSend(nil, s:sub(n,n), false)
        end
        conSend(nil, '\0')
        --]]
        conSend(55, s, false)
        --parseAndSend("[HIDDEN MESSAGE]")
    end
    }
	local leets = {}
	leets.a = '4'
...
dataCmds = {
    ...
    [17]= function()
        ...
        if mpList.notify ~= "false" then
            tpt.notify("TPT Multiplayer", "User " .. con.members[id].name .. " has joined!")
        end
    end,
    [18] = function()
        local id = cByte()
        ...
        if mpList.notify ~= "false" then
            tpt.notify("TPT Multiplayer", "User " .. con.members[id].name .. " has left!")
        end
        con.members[id]=nil
    end,
    [19] = function()
        ...
        if t:find(CurrentNick) and t:find(CurrentNick) > 0 then
            if isAFK then
                conSend(20, 'is AFK right now.', true)
            end
            if mpList.notify ~= "false" then
                tpt.notify("TPT Multiplayer", m .. ":\n\"" .. t .. "\"")
            end
        elseif mpList.notify == "all" then
            tpt.notify("TPT Multiplayer", m .. ":\n\"" .. t .. "\"")
        end
    ...
    [22] = function()
        local msg = conGetNull()
        chatwindow:addline("[SERVER] "..msg, cByte(), cByte(), cByte())
        if mpList.notify ~= "false" then
            tpt.notify("TPT Multiplayer - Server Message", msg)
        end
    end,
    --Mouse Position
    [32] = function()
    ...
    -- Hidden chat
    [55] = function()
        --local id = con.members[cByte()].name
        local s = ''
        local c = ''
        --[[
        for n = 1, 1 do
        ...
        --[[
        local t = os.clock()
        while t > os.clock() - 5 and c ~= '\0' and c ~= 0 do
            c = cByte()
            s = s .. string.char(c)
        end
        --]]
        s = conGetLine()
        chatwindow:addline("[HIDDEN]: " .. s, 50, 255, 255)
    end,
