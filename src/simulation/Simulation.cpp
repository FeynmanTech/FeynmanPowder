...
{
	int rp;
	double tr, tg, tb, ta, colR = colR_, colG = colG_, colB = colB_, colA = colA_;
	double strength = 0.01f;
	/*
	if (IsSmall)
		strength /= 3.;
	*/
	...
	ta /= 255.0; tr /= 255.0; tg /= 255.0; tb /= 255.0;
	colR /= 255.0; colG /= 255.0; colB /= 255.0; colA /= 255.0;
	...
	{
		tr *= 1.0+(colR*strength)*colA;
		tg *= 1.0+(colG*strength)*colA;
		tb *= 1.0+(colB*strength)*colA;
	}
	...
	else if (mode == DECO_EASE)
	{
		/*
		tr = (tr/strength + (colR)) / (1/strength + 1);
		tg = (tg/strength + (colG)) / (1/strength + 1);
		tb = (tb/strength + (colB)) / (1/strength + 1);
		ta = (ta/strength + (colA)) / (1/strength + 1);
		*/
		if (tr > colR) {
			tr -= strength*colA;
			if (tr < colR)
	...
	else if (mode == DECO_SMUDGE)
	{
		...
		{
			float tas = 0.0, trs = 0.0, tgs = 0.0, tbs = 0.0;
...
void Simulation::SetSmall(bool _small) {
	IsSmall = _small;
}

bool Simulation::GetSmall() {
	return IsSmall;
}
	...
	switch (t)
		{
			...
			case PT_BRAY:
				parts[i].life = 30;
				break;
			case 184: // REP: Travis CI fix
				parts[i].life = 10;
				parts[i].tmp = 5;
				break;
			case 185: // REPG: Travis CI fix
				parts[i].life = 10;
				parts[i].temp = 775.0f;
				break;
