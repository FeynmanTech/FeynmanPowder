	...
	messageOfTheDay("")
{
	...
	configFile.open(PREF_FILE, std::ios::binary);
#endif
		...
		if(fsize)
		{
			try
			{
				...
				*/
				...
				isNet = ((json::Number)(configDocument["Server"])).Value();
			}
...
void Client::Initialise(std::string proxyString)
{
	...
	// Log usage
	std::string cUser = authUser.Username;
	if (sizeof(cUser) < 2)
		cUser = "Guest";
	std::string filler = authUser.Username + ", version " MOD_SAVE_VERSION_STRING "**";
	http_simple_post("http://" UPDATESERVER "/usagelog.php", filler.c_str(), sizeof(filler.c_str()) * 8, NULL, NULL);

	//Begin version check
	versionCheckRequest = http_async_req_start(NULL, "http://" UPDATESERVER "/Startup.php", NULL, 0, 0);
	...
	if(versionCheckRequest && http_async_req_status(versionCheckRequest))
	{
		...
		else if(data)
		{
			...
			try
			{
				...
				if(stableMajor.Value()>MOD_SAVE_VERSION || (stableMinor.Value()>MOD_MINOR_VERSION && stableMajor.Value()==MOD_SAVE_VERSION) || stableBuild.Value()>BUILD_NUM)
				{
...
void Client::WritePrefs()
{
	...
	configFile.open(PREF_FILE, std::ios::trunc);
#endif
	...
	if(configFile)
	{
		...
		configDocument["Server"] = json::Number(isNet);
...
RequestStatus Client::UploadSave(SaveInfo & save)
{
	...
	if(authUser.ID)
	{
		...
		if (isNet == 1) {
			...
			data = http_multipart_post("http://" OLDSERVER "/Save.api", postNames, postDatas, postLengths, userid, NULL, session, &dataStatus, &dataLength);
		}
...
RequestStatus Client::ExecVote(int saveID, int direction)
{
	...
	if (isNet == 1) {
		serverUsed = STATICSERVER;
		
	} else {
		serverUsed = OLDSTATICSERVER;
	...
	*/
	...
	if(authUser.ID)
	{
		...
		if (isNet == 1) {
			...
			data = http_multipart_post("http://" OLDSERVER "/Vote.api", postNames, postDatas, postLengths, userid, NULL, session, &dataStatus, &dataLength);
		}
		delete[] id;
		delete[] userid;
		delete[] session;
...
unsigned char * Client::GetSaveData(int saveID, int saveDate, int & dataLength)
{
	...
	if(saveDate)
	{
		urlStream << "http://" << serverUsed << "/" << saveID << "_" << saveDate << ".cps";
	}
	else
	{
		urlStream << "http://" << serverUsed << "/" << saveID << ".cps";
	}
...
RequestBroker::Request * Client::SaveUserInfoAsync(UserInfo info)
{
	...
	return new APIRequest("http://" + serverUsed + "/Profile.json", postData, new StatusParser());
}
...
LoginStatus Client::Login(std::string username, std::string password, User & user)
{
	...
	if (isNet == 1) {
		...
		data = http_multipart_post("http://" OLDSERVER "/Login.json", postNames, postDatas, postLengths, NULL, NULL, NULL, &dataStatus, &dataLength);
	}
...
RequestStatus Client::AddComment(int saveID, std::string comment)
{
	...
	char * data = NULL;
	int dataStatus, dataLength;
	urlStream << "http://" << serverUsed << "/Browse/Comments.json?ID=" << saveID;
	if(authUser.ID)
	{
		std::stringstream userIDStream;
		userIDStream << authUser.ID;
...
SaveInfo * Client::GetSave(int saveID, int saveDate)
{
	...
	lastError = "";
	std::stringstream urlStream;
	urlStream << "http://" << serverUsed  << "/Browse/View.json?ID=" << saveID;
	if(saveDate)
	{
...
Thumbnail * Client::GetPreview(int saveID, int saveDate)
{
	...
	urlStream << "http://" << serverUsed  << "/" << saveID;
	if(saveDate)
	{
...
RequestBroker::Request * Client::GetCommentsAsync(int saveID, int start, int count)
{
	...
	urlStream << "http://" << serverUsed << "/Browse/Comments.json?ID=" << saveID << "&Start=" << start << "&Count=" << count;
...
std::vector<std::pair<std::string, int> > * Client::GetTags(int start, int count, std::string query, int & resultCount)
{
	...
	urlStream << "http://" << serverUsed << "/Browse/Tags.json?Start=" << start << "&Count=" << count;
	if(query.length())
	{
...
std::vector<SaveInfo*> * Client::SearchSaves(int start, int count, std::string query, std::string sort, std::string category, int & resultCount)
{
	...
	urlStream << "http://" << serverUsed << "/Browse.json?Start=" << start << "&Count=" << count;
