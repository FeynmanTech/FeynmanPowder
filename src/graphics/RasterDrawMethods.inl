...
/*
int PIXELMETHODS_CLASS::plottext(int x, int y, const char *s, int r, int g, int b, int a)
{
	if(!strlen(s))
		return 0;
	int width, height;
	...
	for (; *s; s++)
	{
		...
		{
			characterX = plotchar(characterX, characterY, *(unsigned char *)s, r, g, b, a);
		}
	}
	return x;
}
...
/*
int PIXELMETHODS_CLASS::plotchar(int x, int y, int c, int r, int g, int b, int a)
{
	int i, j, w, bn = 0, ba = 0;
	char *rp = font_data + font_ptrs[c];
	w = *(rp++);
	for (j=0; j<FONT_H; j++)
		for (i=0; i<w; i++)
		{
			...
			//sim->create_part(-1, x+i, y+j, PT_DMND);//r, g, b, ((ba&3)*a)/3);
			//(sim->pmap[y+j][x+i]&0xFF) = PT_DMND;
			//(sim->parts[sim->pmap[y+j][x+i]>>8]).dcolour = 0;
			std::cout << y+j << ", " << i+x << "\n";
			std::cout << "pmap: ";
			//assert(sim->pmap != 0);
