...
void Renderer::DrawSigns()
{
	...
	for (i=0; i < signs.size(); i++)
		if (signs[i].text.length())
		{
			...
			int nh = signs[i].height(text);
			splitsign(signs[i].text.c_str(), &type);
			if (type == 'l' && text.length() > 50)
				text = text.substr(0, 50) + "...";
			signs[i].pos(text, x, y, w, h);
			clearrect(x, y, w+1, nh);
			drawrect(x, y, w+1, nh, 192, 192, 192, 255);
			if (!type) {
				...
				if (text.length() > 50)
					drawtext(x+3, y+3, text.substr(0, 50) + "...", 40, 40, 211, 255);
				else
					drawtext(x+3, y+3, text, 40, 40, 211, 255);
			} else {
