...
void ServerSaveActivity::OnDraw()
{
	...
	if(thumbnail)
	{
		if (Client::Ref().GetServer() == 0) {
			...
			g->draw_image(thumbnail, Position.X+(Size.X/2)+((Size.X/2)-thumbnail->Width)/2, Position.Y+(Size.Y-thumbnail->Height)/2, 255);
			g->drawrect(Position.X+(Size.X/2)+((Size.X/2)-thumbnail->Width)/2, Position.Y+(Size.Y-thumbnail->Height)/2, thumbnail->Width, thumbnail->Height, 180, 180, 180, 255);
		}
	}
}
