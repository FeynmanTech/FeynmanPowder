...
void OptionsController::SetSmall(bool state)
{
	model->SetSmall(state);
}

void OptionsController::SetServer(int state)
{
...
void OptionsController::SetScale(bool scale)
{
	if(scale)
	{
		if((ui::Engine::Ref().GetMaxWidth() >= ui::Engine::Ref().GetWidth() * 2 && ui::Engine::Ref().GetMaxHeight() >= ui::Engine::Ref().GetHeight() * 2) || 1) // I want my 2x
