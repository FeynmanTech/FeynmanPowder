...
void OptionsModel::SetSmall(bool state)
{
	sim->SetSmall(state);
	notifySettingsChanged();
}
void OptionsModel::SetServer(int state)
{
	Client::Ref().SetServer(state);
	Client::Ref().SetPref("Server", state);
}
int OptionsModel::GetServer()
{
	return Client::Ref().GetServer();
	//return Client::Ref().GetPrefInteger("Server", 0);
}
bool OptionsModel::GetSmall() {
	return sim->GetSmall();
}
