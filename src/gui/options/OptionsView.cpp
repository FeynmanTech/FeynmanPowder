...
OptionsView::OptionsView():
	...
	class CursorAction: public ui::CheckboxAction
	{
		OptionsView * v;
	public:
		CursorAction(OptionsView * v_){	v = v_;	}
		virtual void ActionCallback(ui::Checkbox * sender){	v->c->SetSmall(sender->GetChecked()); }
	};

	cur = new ui::Checkbox(ui::Point(8, 230), ui::Point(Size.X-6, 16), "Art mode", "");
	cur->SetActionCallback(new CursorAction(this));
	tempLabel = new ui::Label(ui::Point(cur->Position.X+Graphics::textwidth(cur->GetText().c_str())+20, cur->Position.Y), ui::Point(Size.X-28, 16), "\bg- Nicer-looking lines");
	tempLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;	tempLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	AddComponent(tempLabel);
	AddComponent(cur);


	class ServerAction: public ui::CheckboxAction
	{
		OptionsView * v;
	public:
		ServerAction(OptionsView * v_){	v = v_;	}
		virtual void ActionCallback(ui::Checkbox * sender){	v->c->SetServer((int)(sender->GetChecked())); }
	};

	server = new ui::Checkbox(ui::Point(8, 250), ui::Point(Size.X-6, 16), "TPTNet Server", "");
	server->SetActionCallback(new ServerAction(this));
	tempLabel = new ui::Label(ui::Point(server->Position.X+Graphics::textwidth(server->GetText().c_str())+20, server->Position.Y), ui::Point(Size.X-28, 16), "\bg- Use TPTNet server");
	tempLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;	tempLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	AddComponent(tempLabel);
	AddComponent(server);
	...
	fullscreen = new ui::Checkbox(ui::Point(8, 270), ui::Point(Size.X-6, 16), "Fullscreen", "");
	...
	fastquit = new ui::Checkbox(ui::Point(8, 290), ui::Point(Size.X-6, 16), "Fast Quit", "");
	...
	showAvatars = new ui::Checkbox(ui::Point(8, 310), ui::Point(Size.X-6, 16), "Show Avatars", "");
...
void OptionsView::NotifySettingsChanged(OptionsModel * sender)
{
	...
	cur->SetChecked(sender->GetSmall());
	server->SetChecked(sender->GetServer());
