...
int numInputRows = 3; // 4 rows of text
...
public:
	...
	public:
		...
		void ActionCallback(ui::Button * sender)
		{
			...
			{
				prompt->sim->signs[prompt->signID] = sign(sign(gsub(prompt->textField->GetText(), "\\n", "\n"), prompt->signPosition.X, prompt->signPosition.Y, (sign::Justification)prompt->justification->GetOption().second));
			}
			prompt->SelfDestruct();
		}
	};
...
SignWindow::SignWindow(SignTool * tool_, Simulation * sim_, int signID_, ui::Point position_):
	ui::Window(ui::Point(-1, -1), ui::Point(400, 70+17*numInputRows)),
	tool(tool_),
	signID(signID_),
	sim(sim_),
	...
	signMoving(false)
{
	...
	ui::Label * tempLabel = new ui::Label(ui::Point(8, 30 + 17 * numInputRows + 1), ui::Point(40, 15), "Justify:");
	okayButton->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	okayButton->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	AddComponent(tempLabel);

	justification = new ui::DropDown(ui::Point(52, 30 + 17 * numInputRows + 1), ui::Point(50, 16));
	...
	textField = new ui::Textbox(ui::Point(8, 25), ui::Point(Size.X-16, 17*numInputRows), "", "[message]");
	textField->Label::SetMultiline(true);
	textField->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	textField->Appearance.VerticalAlign = ui::Appearance::AlignTop;
	textField->SetActionCallback(new SignTextAction(this));
	//textField->SetLimit(256);
	...
	if(signID!=-1)
	{
		...
		ui::Point position = ui::Point(justification->Position.X+justification->Size.X+3, 30 + 17 * numInputRows + 1);
		...
		position = ui::Point(justification->Position.X+justification->Size.X+3, 30 + 17 * numInputRows + 1)+ui::Point(moveButton->Size.X+3, 0);
...
void SignWindow::DoDraw()
{
	for(std::vector<sign>::iterator iter = sim->signs.begin(), end = sim->signs.end(); iter != end; ++iter)
	{
		...
		if (type == 'l' && text.length() > 50) // Shorten Lua signs to a reasonable size
		...
		if (!type)
			...
			g->drawtext(x+3, y+3, text, 40, 40, 211, 255);
		else
