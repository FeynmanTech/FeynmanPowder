			...
			if(foundSign) {
				...
				if (pos)
				{
					...
					if(type == 'c' || type == 't') {
						...
						std::string luaCode = std::string(buff);
						lua_State *l = luacon_ci->l;
						//luaCode = "local _G = _G local __FUNC={['main']=function() " + luaCode + " end} _G.__EXP = {'os', 'io', 'filesystem', 'fs', 'debug', 'loadstring'} _G.__META = _G for i, v in ipairs(_G.__EXP) do _G.__META[v] = nil end setmetatable(__FUNC, _G.__META) __FUNC.main()"; //Basic sandboxing
						luaCode = "runSandbox([==[" + luaCode + "]==])";
						//luacon_ci->Log(CommandInterface::LogNotice, luaCode);
						const char* luaCodeC = luaCode.c_str();
						if(luaL_loadstring(l, luaCodeC) || lua_pcall(l, 0, 0, 0))
							luacon_ci->Log(CommandInterface::LogError, luacon_geterror());

						//LuaScriptInterface->Command(luaCode); // I have no idea how to do this
					}
				}
			}
		}
...
#ifdef BETA
			currentVersion = MTOS(MOD_SAVE_VERSION) "." MTOS(MOD_MINOR_VERSION) " Beta, Build " MTOS(BUILD_NUM);
#elif defined(SNAPSHOT)
			currentVersion = "Snapshot " MTOS(SNAPSHOT_ID);
#else
			currentVersion = MTOS(MOD_SAVE_VERSION) "." MTOS(MOD_MINOR_VERSION) " Stable, Build " MTOS(BUILD_NUM);
#endif
			/*
			char ver_post[32];
			sprintf(ver_post, "%s*", MOD_SAVE_VERSION_STRING);
			char* whatsnew = http_simple_post("http://" UPDATESERVER "/whatsnew.php", ver_post, sizeof(ver_post),  NULL, NULL);
			*/
			char ver_post[32];
			strncpy(ver_post, MOD_SAVE_VERSION_STRING "*", 32);
			char* whatsnew = http_simple_post("http://" UPDATESERVER "/whatsnew.php", ver_post, 32,  NULL, NULL);

			if (whatsnew == NULL)
				whatsnew = "Unable to get changelog";
