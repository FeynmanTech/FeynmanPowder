...
#include "font_copy.h" //I'm so bad at c++
//#include "graphics/RasterDrawMethods.inl"
#include <math.h>
#include "Config.h"
#include "Format.h"
#include "LuaLuna.h"
...
void LuaScriptInterface::initSimulationAPI()
{
	...
	struct luaL_Reg simulationAPIMethods [] = {
		...
		{"plotchar", simulation_plotchar},
		{"plottext", simulation_plottext},
		{NULL, NULL}
	};
...
int LuaScriptInterface::simulation_rawplotchar(int x, int y, int c, int r, int g, int b, int a)
{
	...
	int i, j, w, bn = 0, ba = 0;
	std::cout << "Font...\n";
	std::cout << sizeof(font_data_copy);
	std::cout << "\n";
	std::cout << sizeof(font_ptrs_copy);
	std::cout << "\n";
	std::cout << sizeof(font_ptrs_copy[c]);
	char *rp = font_data_copy + font_ptrs_copy[c];
	w = *(rp++);
	std::cout << "Foreach pixel...\n";
	for (j=0; j<FONT_H; j++)
		for (i=0; i<w; i++)
		{
			if (!bn)
			...
			if ((((ba&3)*a)/3) > 0) {
				luacon_sim->create_part(-1, x+i, y+j, PT_DMND);//r, g, b, ((ba&3)*a)/3);
				(luacon_sim->parts[luacon_sim->pmap[y+j][x+i]>>8]).dcolour =
					(r*(((ba&3)*a)/3)/255)*65536 + (g*(((ba&3)*a)/3)/255)*256 + (b*(((ba&3)*a)/3)/255) + 255*16777216;
				//255*16777215+(b*(((ba&3)*a)/3)/255)*65535+(g*(((ba&3)*a)/3)/255)*255+;
			}
			//(sim->pmap[y+j][x+i]&0xFF) = PT_DMND;
			//std::cout << y+j << ", " << i+x << "\n";
			//std::cout << "pmap: ";
			//assert(sim->pmap != 0);
			//std::cout << luacon_sim->pmap[y+j][i+x];
			//std::cout << sizeof(sim->pmap[y+j]) / sizeof(sim->pmap[y+j][i+x]);
			//std::cout << "\n";
			ba >>= 2;
			bn -= 2;
		}
...
int LuaScriptInterface::simulation_plotchar(lua_State * l) //int x, int y, int c, int r, int g, int b, int a)
{
	std::cout << "Plotting char...\n";
	int x = lua_tointeger(l, 1);
	int y = lua_tointeger(l, 2);
...
int LuaScriptInterface::simulation_plottext(lua_State * l) //int x, int y, const char *s, int r, int g, int b, int a)
{
	//**
	//int x = 0;
	//**
	...
	int y = lua_tointeger(l, 2);
	//*
	const char * s = (char*)(luaL_optstring(l, 3, ""));
	//*
	int r = luaL_optint(l, 4, 255);
	int g = luaL_optint(l, 5, 255);
	int b = luaL_optint(l, 6, 255);
	...
	for (; *s; s++)
	{
		...
		{
			characterX = simulation_rawplotchar(characterX, characterY, *(unsigned char *)s, r, g, b, a);
		}
		//*/
	}
	//*/
